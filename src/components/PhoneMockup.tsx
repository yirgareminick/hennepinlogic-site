...
function TypingIndicator({ isIncoming = false }: { isIncoming?: boolean }) {
  return (
    <div className={`flex ${isIncoming ? 'justify-end' : 'justify-start'}`}>
      <div className={`px-4 py-3 rounded-2xl ${isIncoming ? 'bg-[#E5E5EA] rounded-tr-md' : 'bg-[#0A84FF] rounded-tl-md'}`}>
        <div className="flex gap-1">
          <div 
            className={`w-1.5 h-1.5 rounded-full ${isIncoming ? 'bg-slate-400' : 'bg-white/70'}`}
  ...
  return (
    <div className="relative">
      ...
      <div className="relative w-[300px] h-[610px] bg-gradient-to-b from-slate-800/70 via-slate-900/80 to-black/90 rounded-[3rem] p-[10px] shadow-[0_25px_90px_rgba(15,23,42,0.55)] border border-white/10 ring-1 ring-white/8 transition duration-500 transform-gpu hover:scale-[0.995]">
        {/* Inner screen */}
        <div className="relative z-10 w-full h-full bg-[#F2F2F7] rounded-[2.5rem] overflow-hidden">
          {/* Dynamic Island */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 w-24 h-6 bg-black rounded-full z-20" />
          
          {/* Status bar */}
          <div className="absolute top-0 left-0 right-0 h-12 z-20 flex items-end justify-between px-8 pb-1">
            <span className="text-[11px] text-slate-800 font-medium">9:41</span>
          ...
          <div className="absolute top-12 left-0 right-0 h-14 bg-white/95 backdrop-blur-sm border-b border-slate-200/80 flex items-center px-4 z-20">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#0A84FF] to-[#2563EB] flex items-center justify-center mr-3">
              <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-slate-900 text-sm font-semibold truncate">{businessName}</p>
                ...
                return (
                  <div key={index} className="flex justify-center message-appear py-1">
                    <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 text-center">
                      <p className="text-amber-600 text-[10px] font-medium tracking-wider">{msg.label}</p>
                      <p className="text-slate-700 text-xs">{msg.text}</p>
                    </div>
                  </div>
                );
              }
                ...
                return (
                  <div key={index} className="flex justify-center message-appear py-1">
                    <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-1.5 flex items-center gap-1.5">
                      ...
                      <p className="text-green-700 text-xs font-medium">{msg.text}</p>
                    </div>
                  </div>
                );
              }
              ...
              if (msg.type === "outgoing") {
                return (
                  <div key={index} className="flex justify-start message-appear">
                    <div className="inline-flex max-w-[85%] bg-[#0A84FF] text-white px-3 py-2 rounded-2xl rounded-tl-md shadow-[0_10px_30px_rgba(10,132,255,0.3)]">
              ...
              if (msg.type === "incoming") {
                return (
                  <div key={index} className="flex justify-end message-appear">
                    <div className="inline-flex max-w-[85%] bg-[#E5E5EA] text-slate-900 px-3 py-2 rounded-2xl rounded-tr-md shadow-[0_8px_18px_rgba(15,23,42,0.15)]">
                      <p className="text-[13px] leading-relaxed break-words whitespace-pre-line">{msg.text}</p>
                    </div>
                  </div>
                );
          ...
          {/* Input area */}
          <div className="absolute bottom-0 left-0 right-0 h-16 bg-white/95 border-t border-slate-200/80 flex items-center px-3 gap-2">
            <div className="flex-1 bg-white border border-slate-200/80 rounded-full px-4 py-2 text-slate-500 text-xs flex items-center">
              <svg className="w-3.5 h-3.5 mr-2 text-[#0A84FF]/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              Auto-responding...
            </div>
          ...
          {/* Home indicator */}
          <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-28 h-1 bg-black/15 rounded-full" />
        </div>
      </div>
    </div>
  );
